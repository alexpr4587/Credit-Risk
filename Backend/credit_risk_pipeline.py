...
# ============================================================
# INGENIERÍA DE ATRIBUTOS Y ENCODING
# ============================================================
...
# =====================================================================
# SECCIÓN 4: EXPECTED LOSS (EL), EXPECTED PROFIT (EP) & APPROVAL MODEL
# =====================================================================

def calculate_el(pd_score: np.ndarray, lgd_hat: np.ndarray, ead_hat: np.ndarray) -> np.ndarray:
    """
    Calcula la Pérdida Esperada (Expected Loss).
    EL = Probabilidad de Default * Pérdida en caso de Default * Exposición
    """
    return pd_score * lgd_hat * ead_hat

def calculate_el_ratio(el: np.ndarray, funded_amnt: np.ndarray) -> np.ndarray:
    """
    Calcula la tasa de Pérdida Esperada respecto al monto del préstamo.
    """
    return el / (funded_amnt + 1e-9)

def calculate_income(funded_amnt: np.ndarray, int_rate: np.ndarray, term: np.ndarray) -> np.ndarray:
    """
    Calcula el ingreso esperado de un préstamo basado en su tasa de interés y plazo.
    Nota: int_rate está en formato porcentaje (ej: 15.5) y term en meses (ej: 36 o 60).
    """
...
def calculate_ep(pd_score: np.ndarray, lgd_hat: np.ndarray, ead_hat: np.ndarray, 
    ...
    """
    Calcula el Beneficio Esperado (Expected Profit).
    EP = (Probabilidad de Pagar * Ingreso) - (Probabilidad de Default * LGD * EAD)
    """
...
def calculate_ep_ratio(ep: np.ndarray, funded_amnt: np.ndarray) -> np.ndarray:
    """
    Calcula el Retorno Esperado (EP Ratio) sobre el monto financiado.
    """
...
class ApprovalModel:
    """
    Modelo de Aprobación basado en rentabilidad esperada (Expected Profit).
    Decide si aprobar un préstamo comparando el EP_ratio contra una 
    tasa de rentabilidad mínima exigida (hurdle_rate).
    """
    ...
    def pd_threshold(self, funded_amnt, int_rate, lgd_hat, ead_hat, term):
        """
        Calcula la Probabilidad de Default (PD) máxima tolerada, o 'punto de equilibrio',
        para lograr exactamente el 'hurdle_rate' fijado.
        """
        income = calculate_income(funded_amnt, int_rate, term)
    ...
    def approve(self, pd_score, lgd_hat, ead_hat, funded_amnt, int_rate, term):
        """
        Devuelve 1 (Aprobar) si el ep_ratio es mayor al hurdle_rate, de lo contrario 0 (Rechazar).
        """
